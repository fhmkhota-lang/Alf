...
function EmberCanvas() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Curve: gentle at top, denser through middle, eases at the end.
  ...
  useEffect(() => {
    ...
    const spawn = (initial = false) => ({
      ...
      warm: Math.random() > 0.82, // a few ember-warm specks
    });
    ...
    const tick = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        if (p.warm) {
          ctx.fillStyle = `rgba(220, 130, 90, ${p.opacity * 0.8})`;
        } else {
          ctx.fillStyle = `rgba(230, 222, 210, ${p.opacity})`;
        }
        ctx.fill();
      }
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      els.forEach((el, i) => {
        ...
        // Use the section centre, clamped — gives a stable "is this the current stop" reading
        const mid = (top + bottom) / 2;
        const dist = Math.abs(centre - mid);
        // Also: if the scroll position is inside the section, prefer it strongly
    ...
    // Re-measure once after a tick in case layout settled later
    ...
    <section ref={ref} className="relative min-h-[100svh] flex flex-col justify-end overflow-hidden">
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 px-6 md:px-16 pb-20 md:pb-32 max-w-5xl">
        ...
        >
          ...
          <span className="block">in the eye of Alf Kumalo</span>
        </motion.h1>
...
function JourneyStop({ stop, index }) {
  ...
  const imgBlur = useTransform(scrollYProgress, [0, 0.25, 0.75, 1], [10, 0, 0, 6]);
  const imgFilter = useTransform(
    imgBlur,
    (b) => `grayscale(1) contrast(1.06) brightness(0.85) blur(${b}px)`
  );

  // Text parallax — different rate than image, creates depth
  const textY = useTransform(scrollYProgress, [0, 1], ['12%', '-8%']);
  const textOpacity = useTransform(scrollYProgress, [0.15, 0.32, 0.78, 0.95], [0, 1, 1, 0.3]);

  // Autonomous "breathing" — gentle, continuous, independent of scroll.
  // Pulled with motion values + useEffect rather than CSS keyframes so it
  // composes with the scroll-driven transforms above.
